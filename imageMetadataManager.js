...
module.exports.updateImageMetadata = (thumbnailImageKey, thumbnailImageId) => {
    console.log('updateImageMetadata');
  
    return getImage(getOriginalFileName(thumbnailImageId)).then(image => {
      const newThumbnails = image.thumbnails;
      newThumbnails.push(thumbnailImageKey);
...
function getOriginalFileName(thumbnailImageId) {

    let fileName = thumbnailImageId.replace('crop_', '');
